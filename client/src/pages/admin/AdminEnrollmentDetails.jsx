...
import { TrendingUp, Search, BookOpen } from 'lucide-react';
...
const AdminEnrollmentDetails = () => {
    ...
    useEffect(() => {
        const fetchEnrollments = async () => {
            try {
                const { data } = await axios.get('/api/analytics/enrollment-details');
                setEnrollments(data);
            } catch (error) {
        ...
        <AdminLayout>
            <div style={{ paddingBottom: '5rem' }}>
                <div style={{ marginBottom: '3rem' }}>
                    ...
                    <p className="page-subtitle" style={{ marginBottom: 0 }}>Breakdown of student enrollments per active course.</p>
                </div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                        {filtered.length > 0 ? (
                            filtered.map((item, index) => (
                                ...
                                >
                                    ...
                                    <div style={{ textAlign: 'right' }}>
                                        <div style={{ fontSize: '1.75rem', fontWeight: 900, color: 'var(--primary)', lineHeight: 1 }}>{item.enrollmentCount}</div>
                                        <div style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em', marginTop: '0.1rem' }}>Students Enrolled</div>
                                    </div>
                                </Card>
                            ))
                        ) : (
                            <div className="card" style={{ padding: '5rem', textAlign: 'center', color: 'var(--text-muted)' }}>
                                ...
                                <p style={{ fontSize: '1.1rem' }}>No enrollment data matches your search.</p>
                            </div>
                        )}
                    </div>
                )}
                    ...
                    <Card style={{ maxWidth: '800px', width: '100%', padding: '2.5rem', position: 'relative', overflow: 'hidden', maxHeight: '90vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
                        ...
                        <div style={{ marginBottom: '2.5rem' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '0.75rem' }}>
                                <div style={{ background: 'var(--primary-light)', color: 'var(--primary)', padding: '0.5rem', borderRadius: '12px' }}>
                                    <BookOpen size={24} />
                                </div>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '1.5rem' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '1.5rem', background: 'linear-gradient(135deg, var(--primary), #8b5cf6)', borderRadius: '20px', color: '#fff', boxShadow: '0 10px 20px -5px rgba(124, 58, 237, 0.3)' }}>
                                <div>
                                    <div style={{ fontSize: '2.5rem', fontWeight: 950, lineHeight: 1 }}>{selectedCourse.enrollmentCount}</div>
                                    <div style={{ fontSize: '0.75rem', fontWeight: 800, opacity: 0.9, textTransform: 'uppercase', marginTop: '0.25rem' }}>Total Students Enrolled</div>
                                </div>
                                        ...
                                        <div key={lvl} style={{ display: 'flex', flexDirection: 'column', background: 'var(--surface-muted)', borderRadius: '12px', border: '1px solid var(--border)', overflow: 'hidden' }}>
                                            ...
                                            >
                                                ...
                                                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                                                    <span style={{ fontWeight: 800, color: 'var(--primary)', background: 'var(--primary-light)', padding: '0.25rem 0.75rem', borderRadius: '8px', fontSize: '0.9rem' }}>{data.count} Students</span>
                                                ...
                                                <div style={{ padding: '1rem', borderTop: '1px solid var(--border)', background: 'var(--surface)', display: 'flex', flexDirection: 'column', gap: '0.5rem', animation: 'fadeIn 0.2s ease' }}>
                                                    {data.students && data.students.length > 0 ? data.students.map(student => (
                                                        ...
                                                        >
                                                            ...
                                                            <div style={{ fontSize: '0.8rem', fontWeight: 600, color: 'var(--primary)' }}>View Profile →</div>
                                                        </div>
                                                    )) : (
                ...
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(10px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000, padding: '2rem' }} onClick={() => setViewingStudent(null)}>
                    <Card style={{ maxWidth: '500px', width: '100%', padding: '2.5rem', position: 'relative', boxShadow: '0 30px 60px -12px rgba(0,0,0,0.25)' }} onClick={e => e.stopPropagation()}>
                        ...
                        <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
                            <div style={{ width: '80px', height: '80px', borderRadius: '24px', background: 'linear-gradient(135deg, var(--primary), #8b5cf6)', margin: '0 auto 1.25rem', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '2.25rem', fontWeight: 900, color: '#fff', boxShadow: '0 10px 20px -5px rgba(124, 58, 237, 0.4)' }}>
                            ...
                            <span className="badge badge-primary" style={{ padding: '0.4rem 1rem' }}>{viewingStudent.rollNumber || 'STU-ID-404'}</span>
                        </div>
